...
def main():
    ...
    # Import the exact sequence creator used during training
    from train_base_model_v2 import create_sequences_zero_lag

    # Create sequences from training data to fit scalers
    X_train, y_train, _ = create_sequences_zero_lag(df_train, feature_cols, target_col, sequence_length=7)
    ...
    from train_base_model_v2 import build_model
    ...
    X_test, y_test, dates = create_sequences_zero_lag(df_test, feature_cols, target_col, sequence_length=7)
    ...
    # Clamp negative flows to 0 (vì Model có thể đoán ra -0.002 do nhiễu số học)
